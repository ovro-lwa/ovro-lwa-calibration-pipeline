...
def finalize_pipeline(context, success, skip_cleanup, no_move_results):
    ...
    if skip_cleanup:
        ...
        # MODIFIED: Search in the new QA_website directory
        qa_website_dir = context.get('website_dir')
        if qa_website_dir and os.path.isdir(qa_website_dir):
            suffixes_to_delete = ['psf', 'residual', 'model', 'dirty']
            files_deleted_count = 0
            logger.info(f"Searching for intermediate WSClean images in {qa_website_dir}...")
            # Search in QA_website and its subdirectories
            for suffix in suffixes_to_delete:
                search_pattern = os.path.join(qa_website_dir, '**', f'*-{suffix}.fits')
            ...
            logger.warning("QA_website directory not found. Skipping cleanup of intermediate images.")
...
def main():
    ...
    context = None
    try:
        # The single-call context setup (now uses your new pipeline_utils.py)
